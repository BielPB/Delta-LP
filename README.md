...
- **Fundo minimalista do Hero**: [HeroBackground.tsx](src/components/three/HeroBackground.tsx) / [HeroOrbitRing.tsx](src/components/three/HeroOrbitRing.tsx) — um contorno fino em forma de estrela de 5 pontas (tubo 3D ao longo de uma curva poligonal, sem imagem/SVG externo) girando ao redor da logo, com leve inclinação 3D que segue o cursor (desktop) ou a inclinação do aparelho (mobile). Sem partículas nem fragmentos no Hero, para não competir com a logo e o texto; cai para uma estrela estática em SVG sem WebGL ou com `prefers-reduced-motion`.
