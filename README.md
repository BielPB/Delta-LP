...
- **Fundo minimalista do Hero**: sem anel, partículas ou fragmentos ao redor da logo, para não competir com o texto. A logo (`RotatingLogoImage`, com leve inclinação 3D que segue o cursor) fica sobre um pedestal 3D em CSS puro (cilindros com gradiente, sem imagens): acabamento branco/acinzentado, sombra de contato e glow ambiente em `var(--color-lime)`, entrada em fade-up e parallax sutil pelo mouse, respeitando `prefers-reduced-motion` ([LogoPedestal.tsx](src/components/ui/LogoPedestal.tsx)).
